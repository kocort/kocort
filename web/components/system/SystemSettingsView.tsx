...
import { Cpu, Database, FileText, Folder, Globe, LayoutDashboard, Wrench } from 'lucide-react';
...
import { LlamaCppSettingsView } from './LlamaCppSettingsView';

type SystemTab = 'dashboard' | 'sandbox' | 'data' | 'tools' | 'network' | 'llamacpp' | 'audit';
    ...
    <div className="flex h-full min-h-0 flex-col bg-white text-zinc-900 transition-colors dark:bg-zinc-950 dark:text-zinc-100">
      <div className="border-b border-zinc-200 bg-white px-6 py-4 pr-20 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="flex flex-wrap items-center gap-2">
          {(['dashboard', 'sandbox', 'data', 'tools', 'network', 'llamacpp', 'audit'] as SystemTab[]).map((tab) => {
      ...
      <div className="min-h-0 flex-1">
        ...
        {activeTab === 'llamacpp' ? <LlamaCppSettingsView /> : null}
