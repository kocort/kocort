...
extern bool llamaAbortCallback(void *user_data);
...
import (
	...
	ggml "github.com/kocort/kocort/internal/llama/ggml/src"
	_ "github.com/kocort/kocort/internal/llama/llama.cpp/common"
	_ "github.com/kocort/kocort/internal/llama/llama.cpp/src"
	_ "github.com/kocort/kocort/internal/llama/llama.cpp/tools/mtmd"
	_ "github.com/kocort/kocort/internal/llama/llama.cpp/tools/mtmd/models"
)
...
type DeviceID struct {
	...
	// management libraries. Only unique within the same Library.
	ID string `json:"id"`
...
func NewContextParams(numCtx int, batchSize int, numSeqMax int, threads int, flashAttention FlashAttentionType, kvCacheType string) ContextParams {
...
type Context struct {
	...
	abortMu     sync.Mutex
	abortHandle cgo.Handle
	abortData   unsafe.Pointer
}

var ErrKvCacheFull = errors.New("could not find a kv cache slot")
var ErrDecodeAborted = errors.New("llama decode aborted")

func (c *Context) Decode(batch *Batch) error {
	// Positive return values does not mean a fatal error, but rather a warning.
	//   0 - success
	//   1 - could not find a KV slot for the batch (try reducing the size of the batch or increase the context)
	//   2 - decode aborted by callback
	// < 0 - error
...
//export llamaAbortCallback
func llamaAbortCallback(userData unsafe.Pointer) C.bool {
	if userData == nil {
		return false
	}
	handleValue := uintptr(*(*C.uintptr_t)(userData))
	if handleValue == 0 {
		return false
	}
	callback := cgo.Handle(handleValue).Value().(func() bool)
	return C.bool(callback())
}
...
func (c *Context) SetAbortCallback(callback func() bool) {
	...
	if data == nil {
		handle.Delete()
		panic("llama abort callback: malloc failed")
	}

	*(*C.uintptr_t)(data) = C.uintptr_t(uintptr(handle))
	c.abortHandle = handle
	c.abortData = data
	C.llama_set_abort_callback(c.c, C.ggml_abort_callback(C.llamaAbortCallback), data)
}
...
func (c *Context) clearAbortCallbackLocked() {
	if c.c != nil {
		C.llama_set_abort_callback(c.c, nil, nil)
	}
	if c.abortData != nil {
		C.free(c.abortData)
		c.abortData = nil
...
// This is the maximum context the model natively supports.
...
// Free releases the sampling context resources.
