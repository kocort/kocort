...
llama_context::llama_context(
    ...
    llama_context_params params) : model(model),
...
llama_context::~llama_context()
{
    // Free the scheduler before backends are destroyed.
    // backends is declared after sched in the class, so its implicit
    // destructor runs first, leaving sched with dangling backend pointers.
    sched.reset();
...
void llama_memory_breakdown_print(const struct llama_context *ctx)
{
    ...
    for (size_t i = 0; i < devices.size(); i++)
    {
        ...
        table_data.push_back({template_gpu,
                              "  - " + name + " (" + desc + ")",
    ...
    // print memory breakdown for host:
    {
        ...
        table_data.push_back({template_other,
                              "  - Host",
                              "", // total
                              "", // free
                              std::to_string(self / MiB),
