...
llama_kv_cache::llama_kv_cache(
    ...
    const layer_reuse_cb &reuse) : model(model), hparams(model.hparams), v_trans(v_trans),
...
ggml_tensor *llama_kv_cache::get_k(ggml_context *ctx, int32_t il, uint32_t n_kv, const slot_info &sinfo) const
{
    ...
    // per-layer head dimension for ISWA (SWA layers may have different head size)
    const uint32_t head_k = (il < (int32_t)hparams.n_layer && hparams.is_swa(il) && hparams.n_embd_head_k_swa > 0)
                                ? hparams.n_embd_head_k_swa
...
ggml_tensor *llama_kv_cache::get_v(ggml_context *ctx, int32_t il, uint32_t n_kv, const slot_info &sinfo) const
{
    ...
    // per-layer head dimension for ISWA (SWA layers may have different head size)
    const uint32_t head_v = (il < (int32_t)hparams.n_layer && hparams.is_swa(il) && hparams.n_embd_head_v_swa > 0)
                                ? hparams.n_embd_head_v_swa
...
ggml_cgraph *llama_kv_cache::build_graph_shift(llm_graph_result *res, llama_context *lctx) const
{
    ...
    for (const auto &layer : layers)
    {
        ...
        // per-layer head dimension for ISWA support
        const uint32_t head_k = (il < hparams.n_layer && hparams.is_swa(il) && hparams.n_embd_head_k_swa > 0)
                                    ? hparams.n_embd_head_k_swa
                                    : hparams.n_embd_head_k;
        ...
        ggml_tensor *k =
            ggml_view_3d(ctx, layer.k,
                         head_k, n_head_kv, get_size() * n_stream,
                         ggml_row_size(layer.k->type, head_k),
