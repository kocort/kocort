        ...
        case LLM_ARCH_QWEN35:
            {
                ...
                switch (hparams.n_layer) {
                    case 24: type = LLM_TYPE_0_8B; break;
            ...
            case LLM_ARCH_GEMMA4:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        const int64_t n_embd_head  = hparams.is_swa(i) && hparams.n_embd_head_k_swa > 0
                                                       ? hparams.n_embd_head_k_swa : n_embd_head_k;
                        const int64_t n_head_kv_i  = hparams.n_head_kv(i);
                        const int64_t n_embd_k     = n_embd_head * n_head_kv_i;
                        const int64_t n_embd_v     = n_embd_head * n_head_kv_i;

                        layer.attn_norm = create_tensor(tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd}, 0);

                        // note: use_alternative_attention (v_proj is optional, if not present, use k_proj)
            ...
            case LLM_ARCH_QWEN35:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (!hparams.is_recurrent(i)) {
                            ...
                            const int64_t qkv_dim = key_dim * 2 + value_dim;
                            layer.wqkv       = create_tensor(tn(LLM_TENSOR_ATTN_QKV,      "weight", i), { n_embd, qkv_dim }, 0);
                            layer.wqkv_gate  = create_tensor(tn(LLM_TENSOR_ATTN_GATE,     "weight", i), { n_embd, value_dim }, 0);
                            layer.ssm_conv1d = create_tensor(tn(LLM_TENSOR_SSM_CONV1D,    "weight", i), { hparams.ssm_d_conv, conv_dim }, 0);
                            layer.ssm_dt     = create_tensor(tn(LLM_TENSOR_SSM_DT,        "bias",   i), { hparams.ssm_dt_rank }, 0);
                            layer.ssm_a      = create_tensor(tn(LLM_TENSOR_SSM_A_NOSCAN,            i), { hparams.ssm_dt_rank }, 0);
