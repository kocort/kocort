...
llm_build_gemma4_iswa::llm_build_gemma4_iswa(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        // per-layer head dimensions (SWA layers may have different head size)
        const int64_t n_embd_head_il = (hparams.is_swa(il) && hparams.n_embd_head_k_swa > 0)
                                         ? hparams.n_embd_head_k_swa : hparams.n_embd_head_k;
        {
            // verify k == v for per-layer head dims
            const int64_t n_embd_head_v_il = (hparams.is_swa(il) && hparams.n_embd_head_v_swa > 0)
                                               ? hparams.n_embd_head_v_swa : hparams.n_embd_head_v;
            GGML_ASSERT(n_embd_head_il == n_embd_head_v_il);
        }
        ...
        // per-layer rotation count: for gemma, n_rot == n_embd_head_k
        const int n_rot_l = (int)((hparams.is_swa(il) && hparams.n_embd_head_k_swa > 0)
                                    ? hparams.n_embd_head_k_swa : n_rot);
        ...
        ggml_tensor * Qcur;
        {
            ...
            Qcur = ggml_reshape_3d(ctx0, Qcur, n_embd_head_il, n_head_il, n_tokens);
        ...
        if (hparams.has_kv(il)) {
            ...
            Kcur = ggml_reshape_3d(ctx0, Kcur, n_embd_head_il, n_head_kv_il, n_tokens);
            Vcur = ggml_reshape_3d(ctx0, Vcur, n_embd_head_il, n_head_kv_il, n_tokens);
        ...
        if (is_moe_layer) {
            ...
            cur_moe = build_moe_ffn(cur_moe,
                    ...
                    il,
                    logits, // probs_in
                    model.layers[il].ffn_gate_up_exps); // gate_up_exps
