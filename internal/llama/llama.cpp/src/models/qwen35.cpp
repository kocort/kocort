...
llm_build_qwen35::llm_build_qwen35(const llama_model &model, const llm_graph_params &params) : llm_graph_context_mamba(params), model(model)
{
    ...
    int il)
{
    ...
    // Qwen3.5 uses a single Q projection that outputs query + gate (2x head size)
    ...
    // Reshape gate to 2D for sigmoid gating
    gate = ggml_cont_2d(ctx0, gate, n_embd_head * n_head, n_tokens);
    cb(gate, "gate_reshaped", il);

    Vcur = ggml_reshape_3d(ctx0, Vcur, n_embd_head, n_head_kv, n_tokens);

    // Apply MRoPE (multi-head rope with sections)
    ...
    int il)
{
    ...
    // Input projections using separate wqkv and wqkv_gate
    auto qkvz = build_qkvz(cur, il);
    ggml_tensor *qkv_mixed = qkvz.first;
    ggml_tensor *z = qkvz.second;

    // Separate beta and alpha projections (unlike qwen3next's combined ssm_beta_alpha)
    ...
    // Choose between chunking and autoregressive delta-net based on sequence length
    ...
    // Extract output and state from concatenated result
...
// Delta-net chunking implementation (adapted from qwen3next)
ggml_tensor *llm_build_qwen35::build_delta_net_chunking(
    ggml_tensor *q,
    ggml_tensor *k,
    ggml_tensor *v,
    ggml_tensor *g,
...
// Delta-net autoregressive implementation (adapted from qwen3next)
ggml_tensor *llm_build_qwen35::build_delta_net_autoregressive(
    ggml_tensor *q,
    ggml_tensor *k,
    ggml_tensor *v,
    ggml_tensor *g,
