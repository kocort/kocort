...
clip_graph::clip_graph(clip_ctx *ctx, const clip_image_f32 &img) : model(ctx->model),
...
struct clip_model_loader
{
    ...
    void load_hparams(clip_model &model, clip_modality modality)
    {
        ...
        // other hparams
        {
            ...
            hparams.has_llava_projector = model.proj_type == PROJECTOR_TYPE_MLP || model.proj_type == PROJECTOR_TYPE_MLP_NORM || model.proj_type == PROJECTOR_TYPE_LDP || model.proj_type == PROJECTOR_TYPE_LDPV2;
            ...
            case PROJECTOR_TYPE_GEMMA4V:
            {
                ...
                get_u32(KEY_PROJ_SCALE_FACTOR, hparams.n_merge, false);
                // SigLIP2 uses rescaled input: 2*x - 1 (maps [0,1] -> [-1,1])
                // Override mean/std so preprocessing does the rescaling
    ...
    void load_tensors(clip_ctx &ctx_clip)
    {
        ...
        for (int il = 0; il < hparams.n_layer; ++il)
        {
            ...
            layer.attn_post_norm = get_tensor(string_format(TN_ATTN_POST_NORM, prefix, il, "weight"), false);
            ...
            layer.ffn_post_norm = get_tensor(string_format(TN_FFN_POST_NORM, prefix, il, "weight"), false);
            ...
            bool is_ffn_swapped = (
                                      ...
                                      model.proj_type == PROJECTOR_TYPE_MLP || model.proj_type == PROJECTOR_TYPE_MLP_NORM || model.proj_type == PROJECTOR_TYPE_LDP || model.proj_type == PROJECTOR_TYPE_LDPV2 || model.proj_type == PROJECTOR_TYPE_QWEN2VL || model.proj_type == PROJECTOR_TYPE_QWEN25VL || model.proj_type == PROJECTOR_TYPE_GLM_EDGE || model.proj_type == PROJECTOR_TYPE_GEMMA3 || model.proj_type == PROJECTOR_TYPE_GEMMA4V || model.proj_type == PROJECTOR_TYPE_IDEFICS3 || model.proj_type == PROJECTOR_TYPE_MINICPMV) &&
...
struct clip_init_result clip_init(const char *fname, struct clip_context_params ctx_params)
{
    ...
    try
    {
        ...
        if (loader.has_audio)
        {
            // Pre-check audio projector type before attempting full load.
            // If the audio projector is unsupported, skip it gracefully
            // so vision can still work.
            std::string audio_proj_type;
            loader.get_string(KEY_AUDIO_PROJ_TYPE, audio_proj_type, false);
            if (audio_proj_type.empty())
            {
                loader.get_string(KEY_PROJ_TYPE, audio_proj_type, false);
            }
            if (clip_projector_type_from_string(audio_proj_type) == PROJECTOR_TYPE_UNKNOWN)
            {
                LOG_WRN("%s: skipping unsupported audio projector type: %s\n", __func__, audio_proj_type.c_str());
            }
            else
            {
...
struct img_tool
{
    ...
    static clip_image_size calc_size_preserved_ratio(const clip_image_size &inp_size, const int align_size, const int longest_edge)
    {
        ...
        auto ceil_by_factor = [f = align_size](float x)
        { return static_cast<int>(std::ceil(x / static_cast<float>(f))) * f; };
    ...
    static clip_image_size calc_size_preserved_ratio(const clip_image_size &inp_size, const int align_size, const int min_pixels, const int max_pixels)
    {
        ...
        auto round_by_factor = [f = align_size](float x)
        { return static_cast<int>(std::round(x / static_cast<float>(f))) * f; };
        auto ceil_by_factor = [f = align_size](float x)
        { return static_cast<int>(std::ceil(x / static_cast<float>(f))) * f; };
        auto floor_by_factor = [f = align_size](float x)
        { return static_cast<int>(std::floor(x / static_cast<float>(f))) * f; };
        ...
        for (int y = 0; y < refined_size.height; y += params.image_size)
        {
            for (int x = 0; x < refined_size.width; x += params.image_size)
            {
                ...
                instructions.slices.push_back(llava_uhd::slice_coordinates{
                    /* x    */ x,
                    /* y    */ y,
                    /* size */ clip_image_size{std::min(params.image_size, refined_size.width - x), std::min(params.image_size, refined_size.height - y)}});
            }
        }
...
int clip_n_output_tokens(const struct clip_ctx *ctx, struct clip_image_f32 *img)
{
    ...
    case PROJECTOR_TYPE_GEMMA4V:
    {
        // gemma4v uses AvgPool2d(kernel=3, stride=3) to reduce 14x14 → 4x4 = 16 tokens
        const int pool_k = 3;
        int n_per_side = (int)sqrt((double)n_patches);          // 14
        int n_pooled_side = (n_per_side - pool_k) / pool_k + 1; // 4
        n_patches = n_pooled_side * n_pooled_side;              // 16
    }
    break;
    default:
...
bool clip_image_batch_encode(clip_ctx *ctx, const int n_threads, const clip_image_f32_batch *imgs_c_ptr, float *vec)
{
    ...
    case PROJECTOR_TYPE_GEMMA4V:
    {
        // set (col, row) patch positions for NaFlex positional embedding + 2D RoPE
...
{
    return ctx->proj_type() == PROJECTOR_TYPE_QWEN2VL || ctx->proj_type() == PROJECTOR_TYPE_QWEN25VL || ctx->proj_type() == PROJECTOR_TYPE_QWEN3VL || ctx->proj_type() == PROJECTOR_TYPE_GLM4V;
}
...
{
    return ctx->proj_type() == PROJECTOR_TYPE_ULTRAVOX || ctx->proj_type() == PROJECTOR_TYPE_QWEN2A || ctx->proj_type() == PROJECTOR_TYPE_GLMA || ctx->proj_type() == PROJECTOR_TYPE_VOXTRAL;
}
