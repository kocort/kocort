...
struct mtmd_context
{
    ...
    mtmd_context(const char *mmproj_fname,
                 ...
                 const mtmd_context_params &ctx_params) : text_model(text_model),
    ...
    void init_vision()
    {
        ...
        {
            // Gemma 4 chat template uses: \n\n<|image|>\n\n
            // The <|image|> token is replaced by embeddings during inference,
            // so we only need the surrounding \n\n as boundary markers.
            img_beg = "\n\n";
            img_end = "\n\n";
        }
...
struct mtmd_tokenizer
{
    ...
    int32_t add_media(const mtmd_bitmap *bitmap)
    {
        if (!bitmap->is_audio)
        {
            ...
            if (
                ctx->slice_tmpl == MTMD_SLICE_TMPL_MINICPMV_2_5 || ctx->slice_tmpl == MTMD_SLICE_TMPL_MINICPMV_2_6 || ctx->slice_tmpl == MTMD_SLICE_TMPL_LLAMA4 || ctx->slice_tmpl == MTMD_SLICE_TMPL_IDEFICS3)
            {
...
bool mtmd_decode_use_non_causal(mtmd_context *ctx)
{
    ...
    if (ctx->ctx_v && clip_get_projector_type(ctx->ctx_v) == PROJECTOR_TYPE_GEMMA4V)
    {
        return true;
    }
    return false;
