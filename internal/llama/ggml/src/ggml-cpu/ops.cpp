    ...
    float *cache, float sin_sign, float theta_scale)
{
    ...
    for (int64_t i0 = 0; i0 < ne0; i0 += 2)
    {
        ...
        float theta = theta_t;
        if (is_imrope)
        { // qwen3vl apply interleaved mrope
            ...
            for (int64_t c_i = c_pkg_end; c_i < c; ++c_i)
            {
                ...
                for (int64_t knl_y = 0; knl_y < p.knl_h; ++knl_y)
                {
                    ...
                    for (int64_t knl_x = 0; knl_x < p.knl_w; ++knl_x)
                    {
                        ...
                        sum += knl_data[(knl_y * p.knl_w + knl_x) * c + c_i] * src_data[(src_y * p.src_w + src_x) * c + c_i];
                    }
                }
                dst_data[c_i] = sum;
            }
                ...
                if (op == GGML_OP_POOL_MAX)
                {
                    ...
                    for (int ky = 0; ky < k1; ++ky)
                    {
                        ...
                        for (int kx = 0; kx < k0; ++kx)
                        {
                            ...
                            const float val = dst->type == GGML_TYPE_F32 ? ((const float *)drowf)[j] : GGML_CPU_FP16_TO_FP32(((const ggml_fp16_t *)drowf)[j]);
                            if (val <= maxval)
                            {
                                continue;
                            }
    ...
    ggml_tensor *dst)
{
    ...
    else if (mode == GGML_SCALE_MODE_BICUBIC)
    {
        ...
        auto weight1 = [a](float x)
        { return ((a + 2) * x - (a + 3)) * x * x + 1; };
        auto weight2 = [a](float x)
