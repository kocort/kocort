...
func (c *Client) SendMessage(ctx context.Context, cfg adapterConfig, msg SendMessageRequest) error {
	...
	if resp.Ret != 0 || resp.ErrCode != 0 {
		return fmt.Errorf("weixin sendmessage api error: ret=%d errcode=%d errmsg=%s", resp.Ret, resp.ErrCode, strings.TrimSpace(resp.ErrMsg))
	}
	return nil
}
