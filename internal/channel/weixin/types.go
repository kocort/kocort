...
// bot API. It handles inbound messages via long-polling and sends outbound
// messages (text, image, video, file) through the iLink endpoints.
package weixin
...
// SessionExpiredErrCode is returned by the iLink API when the bot session has expired.
...
// TextItem holds a text content payload.
type TextItem struct {
	Text string `json:"text,omitempty"`
}

// ImageItem holds an image media payload.
...
// VoiceItem holds a voice media payload.
...
// FileItem holds a file media payload.
...
// VideoItem holds a video media payload.
...
// MessageItem is a single item within a message (text, image, voice, file, video).
