...
func pkcs7Pad(data []byte, blockSize int) []byte {
	...
	for i := len(data); i < len(padded); i++ {
		padded[i] = byte(padding) //nolint:gosec
	}
	return padded
}
...
func pkcs7Unpad(data []byte, blockSize int) ([]byte, error) {
	...
	for i := len(data) - padding; i < len(data); i++ {
		if data[i] != byte(padding) { //nolint:gosec
...
// aesECBPaddedSize returns the ciphertext size after AES-128-ECB with PKCS7
// padding.
...
func encodeCDNMediaAESKey(aesKey []byte) string {
...
// downloadAndDecrypt fetches encrypted bytes from the CDN and decrypts with
// AES-128-ECB.
...
// uploadToCDN encrypts and uploads bytes to the WeChat CDN, returning the
// download param.
func uploadToCDN(cdnBaseURL, uploadParam, filekey string, plaintext, aesKey []byte) (string, error) {
	...
	req, err := http.NewRequest(http.MethodPost, u,
		bytes.NewReader(ciphertext)) //nolint:noctx
	if err != nil {
		return "", err
	}
	req.Header.Set("Content-Type", "application/octet-stream")
	resp, err := (&http.Client{Timeout: 30 * time.Second}).Do(req) //nolint:mnd,gosec
	if err != nil {
